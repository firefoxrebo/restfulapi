...
use RESTAPI\Helpers\Database\DatabaseHandler;
...
class AbstractModel
{
    ...
    public static function getBy($columns, $options = array(), $fetchType = \PDO::FETCH_CLASS)
    {
        $whereClauseColumns = array_keys($columns);
        $whereClauseValues = array_values($columns);
        $whereClause = [];
        for ( $i = 0, $ii = count($whereClauseColumns); $i < $ii; $i++ ) {
            if($whereClauseColumns[$i] === '_results_') continue;
            $whereClause[] = $whereClauseColumns[$i] . ' = "' . $whereClauseValues[$i] . '"';
        }
        $whereClause = implode(' AND ', $whereClause);
        if(array_key_exists('_results_', $columns)) {
            $sql = 'SELECT ' . implode(', ', $columns['_results_']) . ' FROM ' . static::$tableName . '  WHERE ' . $whereClause;
        } else {
